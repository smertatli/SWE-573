def Annotator(processor_name):
    ...
    try:
        ...
        try:
            processed_tweets = pd.read_sql_query('select * from tweet_tagme_annotation limit 1', engine)
        except:
            print('creating annotation table for the first time')
            annotation_table_exists = False

        try:    
            processed_tweets = pd.read_sql_query('select * from df_tweets_processed limit 1', engine)
        except:
        ...
        if processor_table_exists:
            if annotation_table_exists:
                processed_tweets = pd.read_sql_query("""
                        ...
                        left join tweet_tagme_annotation b on a.tweet_tweet_id = b.tweet_tweet_id
                        where b.tweet_tweet_id is null and a.processor_name = '{0}'
                        limit 300""".format(processor_name), engine)
            else:
                conn = engine.connect()
                processed_tweets = pd.read_sql_query("""
                        select a.tweet_tweet_id, a.tweet_text 
            ...
            for index, row in processed_tweets.iterrows():
                print('tagging: ................. ',index,  row['tweet_tweet_id'])
                try:
                    receive = requests.get(query+row['tweet_text'])
                    for a in receive.json()['annotations']:
                        if a['rho'] >= 0.3:
                            row['annotations'] = row['annotations'] + '||' + str(int(a['rho']*100)/100) +'='+a['title']
                except:
