$(document).ready(function() {
  ...
  function initLayout() {
    ...
    var pageName = location.pathname.split('/').find(s => s.includes('.html'));
    
    // Makes secondary menu selected on page load
    $sideNavItem.each(function(index) {
      let $item = $(this);
      let dataItem = $item.data("item");
      let secondaryItems = $sidebarLeftSecondary.find(`[data-parent="${dataItem}"]`);
      ...
      if(pageName) {
        ...
        // console.log($childItem.length ? $childItem : '');
      }
